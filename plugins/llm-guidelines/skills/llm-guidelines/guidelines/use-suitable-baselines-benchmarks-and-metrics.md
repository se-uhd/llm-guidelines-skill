...
Two common metrics used for generation tasks are *BLEU-N* and *pass@k*. *BLEU-N* (Papineni et al. 2002) was originally developed for evaluating machine translation quality by measuring n-gram precision between a candidate and reference text, ranging from![0](https://latex.codecogs.com/png.latex?0 "0") (dissimilar) to![1](https://latex.codecogs.com/png.latex?1 "1") (similar). It has been widely adopted in SE for code generation tasks, though its validity in this context is debatable: n-gram overlap does not capture functional correctness, and syntactically different code can be semantically equivalent (see Challenges below). Code-specific variations such as *CodeBLEU* (Ren et al. 2020) and *CrystalBLEU* (Eghbali and Pradel 2022) attempt to address these limitations by introducing additional heuristics such as AST matching.

The metric *pass@k* reports the likelihood that a model correctly completes a code snippet at least once within![k](https://latex.codecogs.com/png.latex?k "k") attempts. Originally introduced as *success rate at B* by Kulal et al. (2019) (Kulal et al. 2019) and popularized by M. Chen et al. (2021) (M. Chen et al. 2021), it ranges from![0](https://latex.codecogs.com/png.latex?0 "0") (no correct solution in![k](https://latex.codecogs.com/png.latex?k "k") tries) to![1](https://latex.codecogs.com/png.latex?1 "1") (at least one correct solution), with correctness typically defined by passing test cases.

The *pass@k* metric is defined as:

![\text{pass@}k = 1 - \frac{\binom{n-c}{k}}{\binom{n}{k}}\\, \text{where:}](https://latex.codecogs.com/png.latex?%5Ctext%7Bpass%40%7Dk%20%3D%201%20-%20%5Cfrac%7B%5Cbinom%7Bn-c%7D%7Bk%7D%7D%7B%5Cbinom%7Bn%7D%7Bk%7D%7D%5C%2C%2C%20%5Ctext%7Bwhere%3A%7D "\text{pass@}k = 1 - \frac{\binom{n-c}{k}}{\binom{n}{k}}\,, \text{where:}")

-![n](https://latex.codecogs.com/png.latex?n "n") is the total number of generated samples per prompt,
-![c](https://latex.codecogs.com/png.latex?c "c") is the number of correct samples among![n](https://latex.codecogs.com/png.latex?n "n"), and
-![k](https://latex.codecogs.com/png.latex?k "k") is the number of samples drawn.

The choice of![k](https://latex.codecogs.com/png.latex?k "k") depends on the downstream task: *pass@1* is critical for single-suggestion scenarios like code completion, while higher![k](https://latex.codecogs.com/png.latex?k "k") values (e.g.,![2](https://latex.codecogs.com/png.latex?2 "2"),![5](https://latex.codecogs.com/png.latex?5 "5"),![10](https://latex.codecogs.com/png.latex?10 "10")) assess multi-attempt capability (Rozière et al. 2023; Guo et al. 2024; Hui et al. 2024; Li et al. 2023). However, *pass@k* is not a universal metric suitable for all generation tasks. It requires a binary notion of correctness, making the metric appropriate for code synthesis evaluated via unit tests, but unsuitable for open-ended generation tasks such as comment generation, where multiple valid outputs exist.

A complementary perspective from industry practice is *pass<sup>k</sup>* (also written *pass^k*), which measures the probability that *all*![k](https://latex.codecogs.com/png.latex?k "k") trials succeed rather than *at least one* (Anthropic 2025). While *pass@k* increases with![k](https://latex.codecogs.com/png.latex?k "k"), *pass<sup>k</sup>* decreases, making it useful for assessing reliability in deployment scenarios where consistent success matters (e.g., customer-facing agents).

If a study evaluates an LLM-based tool for supporting humans, a relevant metric is the acceptance rate, meaning the ratio of all accepted artifacts (e.g., test cases, code snippets) in relation to all artifacts that were generated and presented to the user. Another way of evaluating LLM-based tools is calculating inter-model agreement, which reveals how much a tool’s performance depends on specific models and versions. Metrics used to measure inter-model agreements include general agreement (percentage), *Cohen’s kappa*, and *Krippendorff’s![\alpha](https://latex.codecogs.com/png.latex?%5Calpha "\alpha")* (see [*Human Validation*](../guidelines/use-human-validation-for-llm-outputs.md) for recommended thresholds and best practices for measuring agreement).
